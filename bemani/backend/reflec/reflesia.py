...
class ReflecBeatReflesia(ReflecBeatBase):
    ...
    def handle_pcb_rb6error_request(self, request: Node) -> Node:
        return Node.void('pcb')

    def handle_pcb_rb6uptime_update_request(self, request: Node) -> Node:
        return Node.void('pcb')

    def handle_pcb_rb6boot_request(self, request: Node) -> Node:
    ...
    def handle_lobby_rb6entry_request(self, request: Node) -> Node:
    ...
    def handle_lobby_rb6read_request(self, request: Node) -> Node:
        root = Node.void('lobby')
        root.add_child(Node.s32('interval', 120))
        root.add_child(Node.s32('interval_p', 120))
    ...
    def handle_lobby_rb6delete_request(self, request: Node) -> Node:
        eid = request.child_value('eid')
        self.data.local.lobby.destroy_lobby(eid)
        return Node.void('lobby')

    def handle_shop_rb6setting_write_request(self, request: Node) -> Node:
        return Node.void('shop')

    def handle_shop_rb6info_write_request(self, request: Node) -> Node:
    ...
    def handle_info_rb6common_request(self, request: Node) -> Node:
        root = Node.void('info')
    ...
    def handle_info_rb6ranking_request(self, request: Node) -> Node:
    ...
    def handle_info_rb6shop_score_ranking_request(self, request: Node) -> Node:
    ...
    def handle_info_rb6pzlcmt_read_request(self, request: Node) -> Node:
        extid = request.child_value('uid')
    ...
    def handle_info_rb6pzlcmt_write_request(self, request: Node) -> Node:
        extid = request.child_value('uid')
        userid = self.data.remote.user.from_extid(self.game, self.version, extid)
        if userid is None:
            # Anonymous comment
    ...
    def handle_player_rb6start_request(self, request: Node) -> Node:
    ...
    def handle_player_rb6end_request(self, request: Node) -> Node:
    ...
    def handle_player_rb6readepisode_request(self, request: Node) -> Node:
    ...
    def handle_player_rb6readscore_request(self, request: Node) -> Node:
        refid = request.child_value('rid')
        userid = self.data.remote.user.from_refid(self.game, self.version, refid)
        if userid is None:
            scores: List[Score] = []
    ...
    def handle_player_rb6selectscore_request(self, request: Node) -> Node:
        extid = request.child_value('uid')
    ...
    def handle_player_rb6total_bestallrank_read_request(self, request: Node) -> Node:
    ...
    def handle_player_rb6delete_request(self, request: Node) -> Node:
        return Node.void('player')

    def handle_player_rb6read_request(self, request: Node) -> Node:
        refid = request.child_value('rid')
    ...
    def handle_player_rb6succeed_request(self, request: Node) -> Node:
        refid = request.child_value('rid')
        userid = self.data.remote.user.from_refid(self.game, self.version, refid)
        if userid is not None:
    ...
    def handle_player_rb6write_request(self, request: Node) -> Node:
